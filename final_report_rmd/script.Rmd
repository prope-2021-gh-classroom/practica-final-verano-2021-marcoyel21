...
\textsc{\LARGE Análisis estadístico de los decesos de COVID19 en la CDMX}\\[1em]
...
\textsc{\large Rodrigo Juárez Jaramillo }\\[1em]

\textsc{\large 145804}\\[1em]
...
```{r, cleaning}
kable(data.frame(sapply(data,class)), col.names = "Clase", booktabs=T,longtable = T)%>% 
  kable_styling(latex_options=c("scale_down","repeat_header"),position = "center")
```

Notamos que todas las variables pertenecen a la clase *character* (con expeción de la edad y los id de cada fila) lo cual habla de que debemos clasificarlas de manera adecuada para aprovechar la información por lo que si queremos aprovechar esas variables, el siguiente paso es asignarles la clase correcta. En el siguiente *chunk* clasificamos las fechas y las variables categóricas de manera correcta.
...
kable(output_missing_data, booktabs=T,col.names = c("Variable","Porcentaje"),
...
Nuestra primera hipótesis esque los valores faltante en *entidad_res* y *municipio_res* en realidad se refieren a personas cuya residencia está en CDMX. Nuestra segunda hipótesis (que convertiremos en supuesto para poder trabajar) esque los valores faltantes en la fecha de defunción corresponden a personas que no han perecido.
...
Lo que notamos en género es que los decesos han sido más en los hombres, aunque las distribuciones se parezcan, notamos que las medias son ligeramente distintas: la de mujeres parece ser en mayor edad y su distribución parece tener una cola más larga hacia la izquierda y una cola con mayor pendiente hacia la derecha con respecto a la distribución de los hombres.
...
En cuanto a comorbidades vemos algunas relaciones muy interesantes: notamos que la hipertensión y diabetes son prevalentes en los decesos (con casi el 40% el 33%). De manera secundaría la obesidad y el tabaquismo aparecen en el 21% y 12% de los decesos aproximadamente.
...
En cuanto a los tiempos, notamos pasa hasta una semana desde le primer sintoma hasta la hospitalización. En cuanto a decesos, notamos que estos ocurren desde el segundo día de la hospitalización
```{r, timing}
times<-data %>% select(tiempo_enf_to_hosp,tiempo_to_death)

a<-ggplot(times, aes(x=tiempo_enf_to_hosp)) + 
  geom_histogram(binwidth=1) +
  ggtitle("Tiempo desde primer sintoma hasta hospitalización")+ xlab("Días") + xlim(0, 20)+ ylim(0, 50000)

b<-ggplot(times, aes(x=tiempo_to_death)) + 
  geom_histogram(binwidth=1) +
  ggtitle("Tiempo desde hospitalización hasta deceso")+ xlab("Días") + xlim(0, 20)+ ylim(0, 7000)
...
La primeras herramientas que usaremos serán aquellas de interpolación. En esta sección probaremos distintos modelos con polinomios de distinto grado con el fin de encontrar aquel que logre la predicción de los decesos de la última semana con el menor error cuadrático medio. El objetivo es poder ajustar un modelo de manera exitosa a la curva de decesos semanales que se presenta a continuación.
...
kable(errors, booktabs=T, col.names = c("Error en decesos para semana 77","Error cuadrático medio general"),
...
La segunda prueba que realizaremos es probar si xiste un efecto diferenciado de la letalidad dependiendo la institución donde se hospitalizó a la persona. A continuación se muestran las tasas de letalidad por tipo de institución de salud.
...
kable(letalidad, col.names = c("Sector","Decesos","Personas atendidas","Tasa de letalidad"), booktabs=T) %>%
...
Finalmente, construiremos un modelo lineal para ver el impacto que tiene cada comorbidad en la probabilidad de perecer devido al COVID19. Primero analizamos la correlación de las variables que usaremos para construir el modelo.

```{r, corr}
df8 <-df8 %>% mutate(deceso=ifelse(is.na(fecha_def),0,1))
cor_data<-df8 %>% select(deceso,edad,diabetes,epoc,asma,inmusupr,hipertension,obesidad,renal_cronica,tabaquismo)
ggcorr(cor_data)

```

Notamos que la edad está altamente correlaciando con la ocurrencia del deceso; la presencia de asma y de tabaquismo, sorprendentemente, no están tan correlacioandos; el resto de las variables está medianamente correlacionado. Asimismo notamos la fuerte correlación entre edad e hipertensión y diabetes e hipertensión.

Con base en estos hallazgos, construimos un modelo lineal para evaluar la frecuencia de morir dependiendo de las distintas comorbidades. En la siguietne tabla mostramos los resultados, cada coeficiente se interpreta como: la presencia de dicha comorbidad en presencia de las demas condiciones (*ceteris paribus*) incrementó la frecuencia de morir por COVID19 una vez infectado en $\beta_i \cdot 100$ puntos porcentuales. Asimismo, en cuanto a la edad, un año adicional incrementó la frecuencia de morir una vez infectado en $\beta_{edad}\cdot 100$ puntos porcentuales.

```{r, lineal, results='asis'}
